...
ff = open(alpha_file,'r')
alpha_dict = {}
for x in ff.readlines():
    tmp_x = x.split()
    if len(tmp_x)==3:
        alpha_dict[tmp_x[-1]]=float(tmp_x[1])
#print(alpha_dict)
ff.close()

for fl in os.listdir(target):
    if os.path.isdir('%s/%s' %(target,fl)):
        for x,y in alpha_dict.iteritems():
            if x==fl:
                ff=open("%s/%s/elec_state" %(target,fl),'a')
                ff.write('alpha_Ex%8.2f' %alpha_dict[fl])
                ff.close()
                break
